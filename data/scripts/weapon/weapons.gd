...
func _ready() -> void:
	...
	arsenal["mk_23"] = weapon.weapon.new(self, "mk_23", 5.0, 999999999999, 999, 12, 40, 1.2);
...
func _weapon(_delta) -> void:
	...
	input["shootboom"] = int(Input.is_action_pressed("mb_right"));
	input["reload"] = int(Input.is_action_pressed("KEY_R"));
	input["zoom"] = int(Input.is_action_pressed("mb_rights"));
	
	arsenal.values()[current]._sprint(character.input["sprint"] or character.input["jump"], _delta);
	
	if input["shootboom"]:
		arsenal.values()[current]._shootboom(_delta);
